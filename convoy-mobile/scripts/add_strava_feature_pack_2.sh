...
# 1. CREATE ACTIVITY DETAILS SCREEN (Analysis)
mkdir -p "app/(tabs)/activity"

cat > "app/(tabs)/activity/[id].tsx" <<'EOF'
...
# 3. LINK FEED TO ACTIVITY DETAIL
# We need to update home.tsx to wrap the card in a Touchable that links to /activity/[id]
sed -i 's/<Card style={s.card}>/<TouchableOpacity activeOpacity={0.9} onPress={() => router.push(`\/(tabs)\/activity\/${item.id}`)}><Card style={s.card}>/' "app/(tabs)/home.tsx"
sed -i 's/<\/Card>/<\/Card><\/TouchableOpacity>/' "app/(tabs)/home.tsx"

rm -rf .expo node_modules/.cache
echo "Done. Feature Pack 2 Installed. Run: npx expo start --tunnel --clear"
